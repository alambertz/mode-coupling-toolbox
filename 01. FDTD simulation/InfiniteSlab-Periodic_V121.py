...
try: #Windows
    spec = importlib.util.spec_from_file_location('lumapi', 'C:\\Program Files\\Lumerical\\v242\\api\\python\\lumapi.py')
...
# ## Define Constructors for Objects
    ...
    props_material= OrderedDict([
        ...
        ("make fit passive", 1),
        ("specify fit range", 1),
        ("wavelength max", lambda_max), 
        ("wavelength min", lambda_min),
    ...
    # Geometry
        ...
        ("x" ,0),
        ("y" ,0),
        ("z max" , pheight+ARthick+headspace + 10e-8),
        ("z min" , -50*nm),
        ("x span" ,boxsize),
        ("y span" ,boxsize),
...
#     properties["source"]=props_source
    ...
    Mzees={    
        1: pheight+ARthick+headspace+50*nm,
        2: -pheight/2,
        3: -25*nm,
        4: -pheight -100*nm,
...
def getFileName():
    global setpam
    NAME="InfSi"+"-periodic"+"-H"+str(round(setpam["pheight"]*1e9)) \
    ...
    NAME+='-'+str(round(setpam["minmesh"]*1e9))+"M-Conf0"
    ...
    for r in radii[:]:
        for p in periodicities[:1]:
            if 2*r >= p:
                continue
            setpam={
                'subthick': 2000  *nm, # Substrate thickness
                'pheight':  h  *nm,
                'pradius':  r  *nm,
