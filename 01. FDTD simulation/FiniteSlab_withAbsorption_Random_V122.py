...
def setupFDTD(properties={}):
    ...
    headspace = properties.get("headspace", 600 * nm)
...
# ## Define Constructors for Objects
    ...
    2:"Honeycomb",
    # 2: "2umHONE",
    3:"HUD_1nm_circle_fromredrawn_l78w78_centered",
    4:"Periodic",
    5:"Honeycomb1",
    6: "random_100-ff38",
    7: "random2_periBC1-ff43-2um",    
    8: "random4",
    9: "random4_ar",
    10: "random5_size5.0um_ff52",
    11: "random5_size5.0um_ff52_ar",
    12:"HUD_1nm_circle_fromredrawn_l150w150_centered",
    13:"HUD_1nm_circle_fromredrawn_l222w222_centered",
    }
    ...
    # GDS Layer. Create AR coating first
    props_gds =OrderedDict([
        ("name",     designname[DESIGN+1]),
        ("cell",     cellname[DESIGN+1]),
        ("layer",    layername[DESIGN+1]),
    ...
    properties["gds"].append(props_gds)
    
            
    # GDS Layer    : Now add the Si pillar
    props_gds2 = OrderedDict([
    ...
    properties["gds"].append(props_gds2)
...
#### Add pillar/hole object AR coating
    ...
    ("override mesh order from material database", True),
    ("mesh order", 8),
    ("override color opacity from material database", 1),
    ("alpha", 0.15)
...
#    fdtd.addrect(properties=props["BR"])
    ...
    for p in props["gds"]:
        addGDS(fdtd, p) # For importing the gds only once
    
    #Add FINE meshes
    if 'dmesh' in setpam.keys():
...
setpam={
    'subthick': .5  *um, # Substrate thickness
    'pheight': 75 * nm,
    'pradius': 75 * nm,
...
def getFileName():
    ...
    NAME+="nmSi-wAbs"+"-withRandom"+str(setpam["DESIGN"]-5)+"-H"+str(round(setpam["pheight"]*1e9)) \
        +'nm'
    ...
    if 'dmesh' in setpam.keys():
        NAME+=str(round(setpam["dmesh"]*1e9))
    # NAME+='-HS'+str(round(setpam["headspace"]*1e9))+"nm"
    NAME+='-400nmfit'
...
heights=[75,175]
radii=[75]
periodicities=[2e3, 5e3, 1e4]

for t in thicknesses[:1]:
    for h in heights[1:]:
        for r in radii[:1]:
            for p in periodicities[1:2]:
                if 2*r >= p:
                    continue
                setpam={
