#!/usr/bin/env python3
"""
plot_fig7_interactive_v2.py -- Interactive Fig 7 (mode-coupling rebuttal).

Based on plot_fig7_interactive_v1.py (= cleaned baseline + slice prompts).
New in v2: the line-shape parameter N can be pinned manually or left to the fit.
  - 'auto' (default): N determined by the fit within [1, 10]
  - number:           N pinned to that value (enter 1 to reproduce clean V88 exactly)

Usage:
    python plot_fig7_interactive_v2.py           # fully interactive
"""
...
siN = scinter.interp1d(n1r[:, 0] * 1000, n1r[:, 1])
siK = scinter.interp1d(n1i[:, 0] * 1000, n1i[:, 1])

# ────────────────────── 2. SETTINGS ──────────────────────
_m = re.search(r'_v(\d+)\.py$', sys.argv[0])
VER = sys.argv[0][-5:-3] if '_V' in sys.argv[0] else (_m.group(1) if _m else 'int')
print('Version', VER)

PALETTE = np.array(['#8750CC', '#FF7F00', '#FFFF00', '#FF3D33', '#FF3D33'])

MOMENTUM_GRID = np.arange(10, 25.5, 0.25)

# v2: N (line-shape parameter) is set interactively in main() and passed as Nfix.
# None -> fit determines N within N_AUTO_RANGE; float -> N pinned to that value.
N_AUTO_RANGE = (1., 10.)
t = 'R'  # normalize power to transmission 'T' or (1-R) 'R'
...
def find_and_extract_lines(arr, x, col=1):
...
def find_and_extract_klines(arr, x, col=1):
...
def gammae_fit(x, center, gami, game, N):
    """Absorption line shape with internal (gami) and external (game) damping."""
    return 4 * gami*game / ((x - center)**2 + (gami + N*game)**2) / Pi

def gammai(lamda):
    """Bulk internal damping [eV] at wavelength lamda [nm], from the Si imaginary index."""
    alpha = 4*Pi/lamda*1e9*siK(lamda)
    return alpha * C0 / siN(lamda)

def calculate_fwhm(x_axis, y_axis, peak_index, left_base, right_base):
...
def fit_GMR(energy, momentum, power, fwhm_thres=2., prominence=3e-3, cov_thres=1e-2):
    ...
    lightline = 2*Pi/eV_to_nm(energy)*1000
    ...
    for peak, prom, left_base, right_base in zip(peak_positions, prominences, left_bases, right_bases):
        ...
        fwhm = calculate_fwhm(x_axis, y_axis, closest_index, left_base, right_base)
    ...
    for peak, _, _ in filtered_peaks:
        ...
        try:
            popt, cov = curve_fit(lorentzian, x_fit, y_fit, p0=initial_guess)
...
def fit_gammaE(energies, power, fwhm_thres=.05, prominence=1e-2, cov_thres=1e-4, Nfix=None):
    ...
    for peak in filtered_peaks:
        ...
        gammai_bulk = gammai(eV_to_nm(peak))*h0/e0
        if Nfix is None:
            n_lo, n_hi, n0 = N_AUTO_RANGE[0], N_AUTO_RANGE[1], 1.0
        else:
            n_lo = n_hi = n0 = float(Nfix)
            n_hi += .001
        initial_guess = [peak, gammai_bulk, gammai_bulk, n0]
        bounds = ([peak/2, 0, 0, n_lo], [peak*2, 1e16, 1e16, n_hi])

        try:
            popt, cov = curve_fit(gammae_fit, x_fit, y_fit, p0=initial_guess, bounds=bounds)
            if popt[2] > 0 and popt[2] < fwhm_thres and cov[0,0] < cov_thres:
                center, gami, game, N = popt
                amplitude = np.max(gammae_fit(x_fit, *popt))
                if center < 1.3 and center > 1.2:
                    print(f"Peak at {center:.3e} eV, A {amplitude:.2e}, gami: {gami:.3e}, game: {game:.3e}, N: {N:.1e}, Regime: {game/gami:.2f}")
                lorentzian_fits.append([round(center,6), amplitude, gami, game, N])
        except RuntimeError:
            pass
    return np.asarray(lorentzian_fits)

def import_full_data(filename, k_min=0, norm_power='R', slice_energies=None, momenta=None, Nfix=None):
    ...
    for line in data_raw:
        ...
        momentum_fits, momentum_peaks = fit_GMR(energy, lam_data[:,1], lam_data[:,2])
    ...
    edic = {}
    momenta_grid = np.arange(10, 25.5, 0.25)
    for mom in momenta_grid:
        subarray = find_and_extract_klines(data, mom, 1)
        edic[mom] = fit_gammaE(subarray[:,0], subarray[:,2], Nfix=Nfix)
...
def colorbar(fig, ax, im, vmax=-1, label='Power (\\%)'):
...
def colorbar_C(fig, ax, im, vmin=None, vmax=None, label='Power (\\%)'):
...
def heatmap(ax, data, slice_energies=None, momenta=None, vmax=0.5, lightline=True, log=False, axv=False, scatter=True):
    ...
    kn = 2*Pi*energies_axis*e0/cnt.h/C0/1e6*n(eV_to_nm(energies_axis)/1000)+0.1
    ...
    for e, col in zip(slice_energies, clrs[:]):
        l = eV_to_nm(e)
        ymax = (2000*Pi/l*siN(l)-ylim0) / (ylim1-ylim0)
        ax.axvline(x=e, ymax=ymax, color=col, linewidth=1)

    for mom, col in zip(momenta, clrs[4:]):
...
def plot_AbsVk(ax, slice_energies=None, data=None, d=None):
    ...
    clrs = PALETTE
    for e, col in zip(slice_energies, clrs[:]):
        subarray = find_and_extract_lines(data, e, 0)
...
def plot_AbsVpe(ax, momenta=None, data=None, d=None):
    ...
    for mom, col in zip(momenta, clrs[4:]):
        ...
        for k in d[mom]:
            center, amplitude, gami, game, N = k
            print("E fit coupling:", center)
            ax.plot(x, 100*gammae_fit(x, center, gami, game, N), '--', color='grey')
...
def plot_Coupling(fig, ax, dic, slice_energies=None, momenta=None, vlines=True):
    ...
    fig, res_ax, cbar2 = colorbar_C(fig, ax, scatter, label=r'$\gamma_e/\gamma_i$')
    ...
    if vlines:
        ...
        xx = np.arange(1, 4, .01)
        ax.plot(xx, 2000*Pi*siN(l)/eV_to_nm(xx), color='black')
...
def plot_figure(data, d, e, slice_energies=None, momenta=None, name='test', vmaxs=None):
    ...
    if momenta is None:
        momenta = [20.5]
    if vmaxs is None:
        vmaxs = [2 for _ in ['placeholder']]

    fig = plt.figure(figsize=(figsize[0]*2, figsize[1]*2), constrained_layout=False)
    ...
    heat_ax, heat_im = heatmap(heat_ax, data, slice_energies=slice_energies, momenta=momenta, vmax=vmaxs[0])
    fig, heat_ax, cbar = colorbar(fig, heat_ax, heat_im, vmaxs[0])

    avk_ax = plot_AbsVk(avk_ax, slice_energies=slice_energies, data=data, d=d)

    avp_ax = plot_AbsVpe(avp_ax, momenta=momenta, data=data, d=e)

    fig, res_ax = plot_Coupling(fig, res_ax, e, slice_energies=slice_energies, momenta=momenta)
    res_ax.set_xlim([1.15, 1.35])
...
def main():
    ...
    if choice.isdigit() and 1 <= int(choice) <= len(candidates):
        filename = candidates[int(choice) - 1]
    else:
        matches = gl(choice)
        filename = sorted(matches)[0] if matches else choice

    # --- photon-energy slices ---
    e_input = input('Photon-energy slices (eV), comma-separated [1.247434]: ').strip() or '1.247434'
    slice_energies = parse_float_list(e_input)

    # --- k_parallel slices ---
    m_input = input("k_parallel slices (um^-1), comma-separated [20.5]: ").strip() or '20.5'
    momenta = parse_float_list(m_input)

    # --- N (line-shape parameter) ---
    ...
    if n_input in ('', 'auto', 'fit'):
        Nfix = None
        n_desc = f"auto (fit within [{N_AUTO_RANGE[0]:g}, {N_AUTO_RANGE[1]:g}])"
    else:
        Nfix = float(n_input)
        n_desc = f"pinned to {Nfix:g}"

    print(f'\nfile={filename}')
    print(f'slice_energies={slice_energies} eV')
    print(f'momenta={momenta} um^-1')
    print(f'N={n_desc}, norm={t}\n')

    data, d, e, peaks = import_full_data(filename, norm_power=t, slice_energies=slice_energies, momenta=momenta, Nfix=Nfix)
    vmaxs = [2]  # colorbar max per file [%] (same as plot_fig7_clean_V188.py)
    plot_figure(data, d, e, slice_energies=slice_energies, momenta=momenta, name='fig_7', vmaxs=vmaxs)
    print('Done')
