...
#### MODESOLVER functions ####
	...
	def getConfinement_slow(self,kyc,kyl,h,m):			#Calculates the confinement factor for a mode in the wave guide, kyc, kyl are decay constants in the
	...
	def getConfinement(self, kyc, kyl, h, m):
		# Precompute constants
		half_h = h / 2
		step_size = h * 0.01
		crange = np.arange(-half_h, half_h, step_size)
		lrange = np.arange(half_h, 10**5, step_size)

		# Vectorized decay function
		f = np.exp(-kyl * (lrange - half_h)) ** 2

		# Core function using vectorization
		if m % 2 == 0:
			core_values = np.cos(kyc * crange) / np.cos(kyc * half_h)
		else:
			core_values = np.sin(kyc * crange) / np.sin(kyc * half_h)

		# Numerical integration using np.trapz
		core = abs(np.trapz(core_values, crange))
		layer = np.trapz(f, lrange)
	...
	def FindBeta(self,X,mod):
		...
		for L in Lambs:
			ca=self.critang(L)
			result=self.TXmode(L,ca,X)[0]
			if result-mod>0:
				sp=0
				def objective(T_angle):
					result = self.TXmode(L, T_angle, X)[0]
					return abs(result - mod)

				res = minimize_scalar(objective, bounds=(ca, Pi/2 - 1e-20), method='bounded', options={'xatol': 10**(-prc)})
				T = res.x
				diff = abs(self.TXmode(L, T, X)[0] - mod)
				print("MODE",X,mod,"Lambda",L,"PRC",diff, "MxMode", int(self.TXmode(L,ca,X)[0]), "Angle - Crit",T-self.critang(L))
				if diff > 10**(-prc/4):
					print("MODE",X,mod,"Lambda",L,"PRC",diff, "MxMode", int(self.TXmode(L,ca,X)[0]), "Angle - Crit",T-self.critang(L))
				else:
					k0=2*Pi/L               
					N=self.TXmode(L,T,X)[1]
					kx=k0*N
					# ki=k0*self.n1Ii(L)/max(self.smk,np.sin(T))
					ki=k0*self.n1Ii(L)*self.getConfinement(kx,k0*0.5*(self.n2Ri(L)+self.n3Ri(L)),self.Thickness,mod)
					arr.append([mod,L,kx,ki,k0])
...
##### FILE EXPORT / IMPORT FUNCTIONS #####    
	...
	def Import(self,name):
		with open("{}.dat".format(name)) as file: #Read thickness of the WG, since this cannot be post-computed
			self.Thickness = float(next(file).split(";")[1].split("=")[1])
			chk=next(file)
		file.close()
	...
	#where propagation constant beta is first found in col 4, and alhpa/2 in col. 5.
		...
		try: 
			...
			print("Mode data not accessible: {}\n Not Found / Wrong holes? ".format(name))
		print(modes)
	...
	def extractModes(self,Mmin=0,Mmax=-1,kmin=0,kmax=1e3):
		S=self
		if Mmax==-1 or Mmax > len(S.mindex):
			Mmax=len(S.mindex)
...
#### PLOT FUNCTIONS ####
	def plDisp(self,name,Mmin=0,Mmax=-1,kmin=0,kmax=-1,noplot=0,ret=0):  #Plotfunction of dispersion/absorption for beta vs. lambda for thickness h (um), name output name
		...
		if kmax==-1:
			...
			ax.plot(x,kmin*x/x,'--',color='darkgrey',label='$k_1$={}'.format(kmin))
			ax.plot(x,kmax*x/x,'-',color='darkgrey',label='$k_2$={}'.format(kmax))
		xyz=S.extractModes(Mmin,Mmax,kmin,kmax)
		if len(xyz)>1:
			...
			ax.plot(x,2*cnt.pi*S.n1Ri(x)/x, '-', color='black', label='h={} $\mu$m, {}'.format(S.Thickness,S.Layers[1]))
	...
	def plDispPeriodic(self,name,Mmin=0,Mmax=-1,kmin=0,kmax=-1,period=0,noplot=0,ret=0):  #Plotfunction of dispersion/absorption for beta vs. lambda for thickness h (um), name output name
		S=self
		Pi=np.pi
		Lambs=[S.Lsta,S.Lend,S.Ldel]
		x=np.asarray(np.arange(*Lambs))
		...
		if Mmax-Mmin>=0:
			...
			ax.plot(x,2*cnt.pi*S.n1Ri(x)/x, '-', color='black', label='h={} $\mu$m {}, L={} $\mu$m'.format(S.Thickness,S.Layers[1],period))
			ax.plot(x,2*cnt.pi*S.n2Ri(x)/x, '-', color='brown', label=S.Layers[0])
			ax.plot(x,2*cnt.pi*S.n3Ri(x)/x, '--', color='blue', label=S.Layers[2])
			ax.set_xlabel('Wavelength ($\mu$m)')
			ax.set_ylabel('K$_{//}$ ($\mu$m$^{-1}$) ')
			ax.set_xlim(Lambs[0],Lambs[1])
			if period > 0:
					# Create the list of diffracton orders
				k_periodic = []
				n_max=int((100)*period/2/Pi)
				for nx in range(0,n_max):
					for ny in range(0,n_max):
						k = np.sqrt( (2*Pi/period*nx)**2 + (2*Pi/period*ny)**2 )
						if k > 0 :
							k_periodic.append( k ) 
				k_periodic=np.unique(np.sort(k_periodic))
				print((k_periodic))
				delta_gap=.5
				
				# delta_k = 2 * np.pi / period
				# k_start, k_end = ax.get_ylim()
				# m_values = np.arange(np.floor(k_start/delta_k)-1, np.ceil(k_end/delta_k)+1)
				# print(m_values)
				last_k=0
				for m in k_periodic:
					k_center = (m-last_k)/2
					...
					rect = patches.Rectangle(
						(Lambs[0], last_k),        # (x,y)
						Lambs[1]-Lambs[0],            # width (full x range)
						delta_k-delta_gap,                     # height (vertical span for each band)
						color='black',
						alpha=0.25,
						edgecolor='none',
						linewidth=0
			...
			fig.set_size_inches(6,4)
			fig.savefig("Dispersion-periodic-{}.png".format(name), bbox_inches='tight', dpi=300)    
			if noplot==1:
				plt.close(fig)
		else:
			print("Error: No Valid Mode Range Provided: {}-{}".format(Mmin/2,Mmax/2))
